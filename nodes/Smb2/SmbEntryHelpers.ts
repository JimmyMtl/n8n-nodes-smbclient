...
const buildClient = async (ctx: IExecuteFunctions): Promise<SmbClientWrapper> => {
	...
	const { host, username, password, domain, share } = (await ctx.getCredentials(
		'smb2Api',
	)) as unknown as Smb2Credentials;
	debug('Connecting to //%s/%s as (%s\\%s)', host, share, domain ?? '', username);
	...
	if (source === 'binary') {
		...
		if (!item.binary || !item.binary[binProp]) {
			throw new NodeOperationError(
				ctx.getNode(),
				`Binary property "${binProp}" not found on item ${i}`,
			);
		}
